...
| 7.15 | Consent survives the PAGE BOUNDARY an OAuth redirect creates: ticked on one page load, read back and recorded on the next | WALKED 2026-08-11 (production, with `js/consent.js` deliberately delayed on the return load) |
| 7.16b | The literal Google / LinkedIn login page | **BLOCKED, not skipped.** Needs a real provider account. The only ones available are Chris's own, and walking as a real person is forbidden (see `UNVERIFIED.md`). Everything on our side of that handshake is covered by 7.15 and 7.19. |
